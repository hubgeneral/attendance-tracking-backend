...
namespace attendance_tracking_backend.GraphQL
{
    ...
    public class DashboardQuery
    {
        ...
        public IQueryable<RequestLog> GetRequestLogs(DateOnly day, DatabaseContext dbcontext)
        {
           DateOnly today = DateOnly.FromDateTime(DateTime.UtcNow);
           var results = dbcontext.RequestLogs.Where(r => r.CurrentDate == today);

            return results;
        }

        public WorkingHours WorkHoursSummary(DateOnly day, [Service] DatabaseContext dbcontext)
        {
            DateOnly today = DateOnly.FromDateTime(DateTime.UtcNow);
            var TotalNumberOfAttendances = dbcontext.Attendances.Where(r => r.CurrentDate == today).Count();
        ...
        //most hours worked
        public IQueryable<MostHoursWorkedByEmployee> MostHoursWorked(DateOnly day, [Service] DatabaseContext dbcontext)
        {
            

            var top5Employees = dbcontext.Attendances
           .Include(a => a.User) // Include related AppUser for EmployeeName
           .Where(a => a.CurrentDate == day && a.TotalHoursWorked != null).GroupBy(a => new { a.AppUserId, a.User!.EmployeeName })
        ...
        //most off hours
        public IQueryable<MostHoursWastedByEmployee>  MostWastedHours(DateOnly day, [Service] DatabaseContext dbcontext)
        {
            const decimal standardWorkingHours = 8m; // full workday

            var top5Employees = dbcontext.Attendances
                .Include(a => a.User) // Load employee info
                .Where(a => a.CurrentDate == day && a.TotalHoursWorked != null)
        ...
        // most late employees
        public IEnumerable<LateEmployees> GetLateEmployees(DateOnly day , [Service] DatabaseContext dbcontext)
        {
            ...
            var lateEmployees = dbcontext.Attendances
                .Include(a => a.User)
                .Where(a => a.CurrentDate == day && a.ClockIn != null)
        ...
        public IEnumerable<PunctualEmployees> GetPunctualEmployees(DateOnly day, [Service] DatabaseContext dbcontext)
        {
